...
#define CH_SERVER   "mqtt3.thingspeak.com"
#define CH_PORT     (1883)
#define CH_NUMBER   (xxx)
#define CH_USERNAME "xxx"
#define CH_CLIENTid "xxx"
#define CH_PASSWORD "xxx"
...
void setup() {
  ...
  MQTT.setServer( CH_SERVER, CH_PORT );
...
void loop() {
  ...
  // reading LDR
  int LDR = READ_LDR();
  // set messages for sending by MQTT to ThingSpeak
  String DATA   = "field1=";
  DATA  +=  TEMP;
  DATA  +=  "&field2=";
  DATA  +=  LDR;
  if ( MQTT.connect( CH_CLIENTid, CH_USERNAME, CH_PASSWORD ) ) {
    Serial.println("MQTT CONNECTED");
    String TOPIC  = "channels/" + String(CH_NUMBER) + "/publish";
    // publish messages to ThingSpeak 
    if (MQTT.publish(TOPIC.c_str(), DATA.c_str())) {
      Serial.println("Temperature (C) >> " + String(TEMP));
      Serial.println("LDR values >> " + String(LDR));
    }
  }
  delay(5000);
}
